...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/GRCoinParams"
else
    PARAMS_DIR="$HOME/.grcoin-params"
fi
...
DOWNLOAD_URL="https://github.com/GRCoin-Coin/Core-Wallet/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with GRR_DISABLE_SOMETHING=1
GRR_DISABLE_WGET="${GRR_DISABLE_WGET:-}"
#GRR_DISABLE_IPFS="${GRR_DISABLE_IPFS:-}"
GRR_DISABLE_CURL="${GRR_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$GRR_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$GRR_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$GRR_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the GRCoin parameters!
...
This script will fetch the GRCoin SNARK parameters and verify their
...
EOF
    ...
    if ! [ -d "$PARAMS_DIR" ]; then
        ...
        cat >>"$README_PATH" <<EOF
This directory stores common GRCoin SNARK parameters. Note that it is
