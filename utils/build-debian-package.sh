...
PACKAGE_VERSION=$($SRC_PATH/src/grcoind --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/grcoind $DEB_BIN
cp $SRC_PATH/src/grcoin-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/grcoin-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/grcoind.1 $DEB_MAN
cp $SRC_DOC/man/grcoin-cli.1 $DEB_MAN
cp $SRC_DOC/man/grcoin-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/grcoind.bash-completion $DEB_CMP/grcoind
cp $SRC_PATH/contrib/grcoin-cli.bash-completion $DEB_CMP/grcoin-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/grcoind.1
gzip --best -n $DEB_MAN/grcoin-cli.1
gzip --best -n $DEB_MAN/grcoin-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/grcoind $DEB_BIN/grcoin-cli
