...
void static GRCoinMiner(const CChainParams& chainparams)
{
    LogPrintf("GRCoinMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("grcoin-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("GRCoinMiner -- Wallet not available\n");
    }
#endif
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get()) {
                LogPrintf("GRCoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("GRCoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        LogPrintf("GRCoinMiner -- terminated\n");
        throw;
    } catch (const std::runtime_error& e) {
        LogPrintf("GRCoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateGRCoins(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++) {
        minerThreads->create_thread(boost::bind(&GRCoinMiner, boost::cref(chainparams)));
    }
    return (numCores);
}
