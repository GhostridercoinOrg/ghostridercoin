...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help")) {
        ...
        std::string strUsage = strprintf("%s grcoin-tx utility version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                               "Usage:\n"
                               "  grcoin-tx [options] <hex-tx> [commands]  Update hex-encoded grcoin transaction\n" +
                               "  grcoin-tx [options] -create [commands]   Create hex-encoded grcoin transaction\n" +
                               "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded grcoin transaction
