...
const char* const BITCOIN_CONF_FILENAME = "grcoin.conf";
const char* const BITCOIN_PID_FILENAME = "grcoind.pid";
...
public:
    ...
    static inline std::pair<bool, std::string> GetArg(const ArgsManager& am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "grcoind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GRCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GRCoinCore
    // Mac: ~/Library/Application Support/GRCoinCore
    // Unix: ~/.grcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GRCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GRCoinCore";
#else
    // Unix
    return pathRet / ".grcoincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty grcoin.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure GRCoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("GRCoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The GRCoin Core developers";
    }
