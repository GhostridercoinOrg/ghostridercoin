...
This document provides guidelines for internationalization of the GRCoin Core software.
...
This adds `<numerusform>`s to the respective `.ts` file, which can be translated separately depending on the language. In English, this is simply:
    ...
    <message numerus="yes">
        <source>%n active connection(s) to GRCoin network</source>
        <translation>
            <numerusform>%n active connection to GRCoin network</numerusform>
            <numerusform>%n active connections to GRCoin network</numerusform>
        </translation>
    </message>
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `grcoin_en.ts` remains unchanged.
