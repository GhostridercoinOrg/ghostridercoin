...
# grcoin_hash
RUN git clone https://github.com/GRCoin-Coin/Core-Wallet_hash
RUN cd grcoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} grcoin
RUN useradd -u ${USER_ID} -g grcoin -s /bin/bash -m -d /grcoin grcoin
...
RUN mkdir /grcoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /grcoin-src && \
